...
module.exports = new Scenes.WizardScene("addPhraseScene",
    async ctx => {
        ...
        await ctx.reply("Теперь, зная, как пользоваться форматированием, отправьте цитату с нужными тегами", { reply_markup: { inline_keyboard: [[{ text: "Отмна", callback_data: "cancel" }]] } });
    ...
    async ctx => {
        ...
        await ctx.replyWithHTML(`Цитата будет выглядеть вот так:\n\n${ctx.message.text}\n\nПодтверждаете добавление?`, { reply_markup: { inline_keyboard: [[{ text: "Да", callback_data: "confirmAdding" }], [{ text: "Назад", callback_data: "toPreviousStep" }]] } })
        ctx.scene.session.state.phrase = ctx.message.text;
        return ctx.wizard.next()
    },
    ctx => {
        if (!["confirmAdding", "toPreviousStep"].includes(ctx?.callbackQuery?.data)) return ctx.reply("Выберите одну из кнопок")
        if (ctx.callbackQuery.data == "toPreviousStep") return ctx.scene.reenter();
        ctx.reply("Цитата успешно добавлена")
...
{
    ctx.reply("Добавление новой цитаты отменено")
    ctx.scene.leave()
}
